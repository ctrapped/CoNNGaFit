...
class NeuralNetwork(nn.Module):
    ...
    def forward(self, x, saveLatentImages=False):
        ...
        x = x + featureMap3[:,:,:,0:37,0:37] #Feature forwarding from left convolutional -> deconvolutional wing
        #x = x + featureMap3[:,:,0:9,0:7,0:7] #Feature forwarding from left convolutional -> deconvolutional wing
        if saveLatentImages: SaveSummedSpectralChannels(x,5)
            
        x = self.dc_layer2(x)
       # print("Shape of x=",np.shape(x),"... Shape of featureMap2=",np.shape(featureMap2))

        x = x + featureMap2[:,:,0:9,1:74,1:74] #Cropped Feature forwarding
        #x = x + featureMap2[:,:,1:18,1:14,1:14] #Cropped Feature forwarding
       ...
       # print("Shape of x=",np.shape(x),"... Shape of featureMap1=",np.shape(featureMap1))
        ...
        x = x + featureMap1[:,:,1:18,2:147,2:147] #Cropped Feature forwarding
        #x = x + featureMap1[:,:,2:35,2:27,2:27] #Cropped Feature forwarding
        if saveLatentImages:
