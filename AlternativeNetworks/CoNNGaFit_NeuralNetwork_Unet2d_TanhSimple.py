...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, downsample, kernel1):
        ...
        if downsample: #reduce dimensionality
            self.conv1 = nn.Conv2d(in_channels,out_channels,kernel_size=kernel1,stride=(2,2),padding=1)
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels, out_channels,kernel_size=(1,1),stride=(2,2)),
                nn.BatchNorm2d(out_channels)
            )
        else:
            self.conv1 = nn.Conv2d(in_channels,out_channels,kernel_size=kernel1,stride=(1,1),padding=1)
            self.shortcut = nn.Sequential()
            
            
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=kernel1, stride=(1,1),padding=1)
...
class Upsample(nn.Module):
    def __init__(self, in_channels, out_channels, upsample,kernel1):
        super().__init__()
        self.upconv1 = nn.ConvTranspose2d(in_channels,out_channels,kernel_size=kernel1,stride=(2,2),padding=1)
...
class NeuralNetwork(nn.Module):
    def __init__(self, nChan, nFilt0, kernel0, kernel1, nFC):
        ...
        self.layer0 = nn.Sequential(
            nn.Conv2d(nChan,nFilt0,kernel_size=kernel0,stride=(2,2),padding=(3,3)),
        ...
        ##  Residual Layers  ##
        self.layer1 = ResidualBlock(nFilt0,nFilt0,downsample=False,kernel1=kernel1) ##Downsample

        
        self.layer2 = ResidualBlock(nFilt0,2*nFilt0,downsample=True,kernel1=kernel1)
        ...
        self.layer4 = ResidualBlock(4*nFilt0,8*nFilt0,downsample=True,kernel1=kernel1)
        #######################
        
        ## Deconvolutional Layers ##
        self.upsample1 = Upsample(8*nFilt0,4*nFilt0,upsample=True,kernel1=kernel1)
        self.dc_layer1 = DeconvBlock(8*nFilt0,4*nFilt0,upsample=True,kernel1=kernel1)
        self.upsample2 = Upsample(4*nFilt0,2*nFilt0,upsample=True,kernel1=kernel1)
        self.dc_layer2 = DeconvBlock(4*nFilt0,2*nFilt0,upsample=True,kernel1=kernel1)
        self.upsample3 = Upsample(2*nFilt0,nFilt0,upsample=True,kernel1=kernel1)
        self.dc_layer3 = DeconvBlock(2*nFilt0,nFilt0,upsample=True,kernel1=kernel1)     
        ...
        if self.nFC>0:
            if nChan==3: nif=576
            if nChan==77: nif=2304
            self.fc0 = nn.Linear(in_features=nif , out_features=nFC)
    ...
    def forward(self, x, saveLatentImages=False):
        ...
        #x = torch.reshape(x,(nBatch,nSpec,nX,nY))
        ...
        x = self.upsample1(x)
        x = torch.cat([x,featureMap3],axis=1)
        x = self.dc_layer1(x)
        ...
        x = self.upsample2(x)
        x = torch.cat([x,featureMap2[:,:,0:9,:,:]],axis=1)
        x = self.dc_layer2(x)
        x = x + featureMap2[:,:,:,:] #Cropped Feature forwarding
        if saveLatentImages: SaveSummedSpectralChannels(x,5)

        x = self.upsample3(x)
        x = torch.cat([x,featureMap1[:,:,1:18,0:9,0:9]],axis=1)
        x = self.dc_layer3(x)
        x = x + featureMap1[:,:,0:9,0:9] #Cropped Feature forwarding
        if saveLatentImages:
