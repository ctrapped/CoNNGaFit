...
trainingDir = 'CoNNGaFitData\\'+inputDirName+'\\i'+str(inclination)+'\\training\\finalTrainingSets\\'+input_subdir+'\\training_annotations_RC'+sampleSuffix+'.csv'
validationDir = 'CoNNGaFitData\\'+inputDirName+'\\i'+str(inclination)+'\\training\\finalTrainingSets\\'+input_subdir+'\\validation_annotations_RC'+validationSuffix+'.csv'
testingDir = 'CoNNGaFitData\\'+inputDirName+'\\i'+str(inclination)+'\\training\\finalTrainingSets\\'+input_subdir+'\\validation_annotations_RC'+testingSuffix+'.csv'
...
imageOutput_input = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\input\\RC'+sampleSuffix
imageOutput=imageOutput_input
imageOutput_prediction = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\prediction\\RC'+outputSuffix+'_'
imageOutput_training = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\training\\RC'+outputSuffix+'_'
imageOutput_training_input = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\training\\input\\RC'+outputSuffix+'_'
imageOutput_comparison = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\comparisons\\'+subFolder+'\\RC'+outputSuffix+'_'
imageOutput_comparison_finalTest = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\comparisons\\'+subFolder+'\\testingSet\\RC'+outputSuffix+'_'

imageOutput_prediction_finalTest = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\testing\\RC'+sampleSuffix
imageOutput_input_finalTest = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\testing\\input\\RC'+sampleSuffix

diagnosticOutput = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\images\\comparisons\\'+subFolder+'\\RC'+outputSuffix

modelOutputPath = 'CoNNGaFitData\\'+dirName+'\\i'+str(inclination)+'\\trainedNetworks\\RC_Unet18_FullSpec'+outputSuffix+'_epoch'+str(epochs)#+'.pt'
...
def MakeImage(data,output,vmin,vmax,Nsnap):
    ...
    vmin=-vmax
    plt.imshow(image ,cmap='seismic',vmin=vmin,vmax=vmax)
    plt.colorbar();
...
def MakeCompImage(data_pred,data_input,data_image,output,vmin,vmax,Nsnap):
    ...
    print("nBins=",nBins)
    binned_data_pred,binedge,binnum = stats.binned_statistic_dd(rmag.flatten(),image_pred.flatten(),"mean",nBins,range=[binRange])
    binned_data_input,binedge,binnum = stats.binned_statistic_dd(rmag.flatten(),image_input.flatten(),"mean",nBins,range=[binRange])
    maxPlot = np.max([-np.min(binned_data_pred),-np.min(binned_data_input),np.max(binned_data_pred),np.max(binned_data_input)])
    plt.plot(np.linspace(0,np.max(rmag),nBins) , binned_data_input , 'k',lw = 2)
    plt.plot(np.linspace(0,np.max(rmag),nBins) , binned_data_pred  , 'r--' ,lw = 2)
    plt.legend(['Input','Prediction'])
    plt.plot(np.linspace(0,np.max(rmag),nBins) , binned_data_pred * 0 , 'k--' ,lw = 1)
    plt.xlabel('Radius')
    plt.ylabel('Rot. Vel. [km/s]')
    plt.ylim([-maxPlot*1.05,maxPlot*1.05])
    
    
    vmax = np.max( [-np.min(image_pred) , np.max(image_pred), -np.min(image_input) , np.max(image_input)])
    vmin=-vmax
    plt.subplot(2,2,1)
    plt.imshow(image_input ,cmap='seismic',vmin=vmin,vmax=vmax)
    plt.title("Rot. Vel. Input")
    #plt.colorbar();
    plt.gca().axes.xaxis.set_ticks([])
    plt.gca().axes.yaxis.set_ticks([])
    plt.subplot(2,2,2)
    plt.imshow(image_pred ,cmap='seismic',vmin=vmin,vmax=vmax)
    plt.colorbar(label = '[km/s]');
    plt.gca().axes.xaxis.set_ticks([])
    plt.gca().axes.yaxis.set_ticks([])
    plt.title("Rot. Vel. Prediction")
    plt.savefig(output)
    plt.close()
    
def MakeCompScatterPlot(data_pred,data_input,output,defineStandardError=False):

    points_pred = data_pred.flatten()
    points_input = data_input.flatten() 
...
def MakeCompScatterPlot_v2(data_pred,data_input,output,defineStandardError=False):
    ...
    for rMin in range(0,nPix-step):
      ...
      points_pred = netMF_pred[ii,:].flatten()
      points_input = netMF_input[ii,:].flatten() 
    ...
    points_pred = netMF_pred.flatten() #all rings now
    points_input = netMF_input.flatten()     
    xmin = min(np.min(points_pred),np.min(points_input))
    xmax = max(np.max(points_pred),np.max(points_input))
...
def CreateBasicPlots(data,output,Nsnap):
    ...
    plt.figure()
    plt.plot(np.linspace(0,np.max(rmag),nBins) , binned_data , lw = 1.5)
    plt.xlabel('Radius')
    plt.savefig(output)
    plt.close()
...
def CreateBasicCompPlots(data_pred,data_input,output,Nsnap):
    ...
    plt.figure()
    plt.plot(np.linspace(0,np.max(rmag),nBins) , binned_data_input , 'r--',lw = 1.5)
    plt.plot(np.linspace(0,np.max(rmag),nBins) , binned_data_pred  , 'k' ,lw = 1.5)
    plt.legend(['Input','Prediction'])
    plt.xlabel('Radius')
    plt.savefig(output)
    plt.close()
