...
class NeuralNetwork(nn.Module):
    def __init__(self, nFilt0, kernel0, kernel1, nFC):
        ...
        if self.nFC>0:
            self.fc0 = nn.Linear(in_features=128*nFilt0 , out_features=1400)
            self.relu0 = nn.ReLU()
            self.fc1 = nn.Linear(in_features=1400 , out_features=720)
            self.relu1 = nn.ReLU()
            self.fc2 = nn.Linear(in_features=720 , out_features=264)
            self.relu2 = nn.ReLU()
            #self.relu0 = nn.Tanh()
            #self.relu0 = nn.ReLU()

            self.fc3 = nn.Linear(in_features=264,out_features=model_size)
        else:
            self.fc3 = nn.Linear(in_features=128*nFilt0,out_features=model_size)
