...
class DeconvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, upsample,kernel1):
        ...
        if upsample: #increase dimensionality
            self.conv1 = nn.ConvTranspose2d(in_channels,out_channels,kernel_size=kernel1,stride=(2,2),padding=1)
            self.shortcut = nn.Sequential(
                nn.ConvTranspose2d(in_channels, out_channels,kernel_size=(1,1),stride=(2,2)),
                nn.BatchNorm2d(out_channels)
            )
        else:
            self.conv1 = nn.ConvTranspose2d(in_channels,out_channels,kernel_size=kernel1,stride=(1,1),padding=1)
        ...
        self.conv2 = nn.ConvTranspose2d(out_channels, out_channels, kernel_size=kernel1, stride=(1,1),padding=1)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.bn2 = nn.BatchNorm2d(out_channels)
...
class NeuralNetwork(nn.Module):
    def __init__(self, nChan, nFilt0, kernel0, kernel1, nFC):
        ...
        self.dc_layer2=nn.Sequential(
            DeconvBlock(8*nFilt0,4*nFilt0,upsample=True,kernel1=kernel1), ##Upsample
            DeconvBlock(4*nFilt0,2*nFilt0,upsample=False,kernel1=kernel1)
        )
        
        self.dc_layer3=nn.Sequential(
            DeconvBlock(4*nFilt0,2*nFilt0,upsample=True,kernel1=kernel1), ##Upsample
            DeconvBlock(2*nFilt0,nFilt0,upsample=False,kernel1=kernel1)
        )        
        ...
        if self.nFC>0:
            if nChan==3: nif=576*2
            if nChan==77: nif=2304*2
            ...
            self.fc1 = nn.Linear(in_features=256*nFilt0,out_features=model_size)
    ...
    def forward(self, x, saveLatentImages=False):
        ...
        x = torch.cat([x,featureMap2],axis=1)
        if saveLatentImages: SaveSummedSpectralChannels(x,5)
            
        x = self.dc_layer3(x)
        #x = x + featureMap1[:,:,0:9,0:9] #Cropped Feature forwarding
        x = torch.cat([x,featureMap1[:,:,0:9,0:9]],axis=1)
