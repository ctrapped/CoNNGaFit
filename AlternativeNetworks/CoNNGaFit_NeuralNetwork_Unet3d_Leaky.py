...
class ResidualBlock(nn.Module):
    ...
    def forward(self,x):
        ...
        x = nn.LeakyReLU()(self.bn1(self.conv1(x))) #Try replacing with leaky
...
class NeuralNetwork(nn.Module):
    def __init__(self, nFilt0, kernel0, kernel1, nFC):
        ...
        if self.nFC>0:
            ...
            #self.relu0 = nn.Tanh()
            #self.fc1 = nn.Linear(in_features=nFC,out_features=nFC)
           # self.relu1 = nn.Tanh()
            self.fc2 = nn.Linear(in_features=nFC,out_features=model_size)
